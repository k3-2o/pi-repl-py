...
describe("previewCell: Python", () => {
	run([
		{
			name: "bash positional command wins",
			code: 'out = bash("ls -la")\nout.stdout',
			kind: "shell",
			text: "ls -la",
		},
		{
			name: "bash keyword command",
			code: 'bash(command="git status --short", timeout=30)',
			kind: "shell",
			text: "git status --short",
		},
		{
			name: "IPython bang shell",
		...
		{
			name: "read path",
			code: 'cfg = read("package.json")\njson.loads(cfg)',
			kind: "ts",
			text: "read package.json",
		},
		{
			name: "write path",
			code: 'write("/tmp/report.md", "# summary")',
			kind: "ts",
			text: "write /tmp/report.md",
		},
		{
			name: "edit path",
			code: 'edit("src/main.py", old_text="a", new_text="b")',
			kind: "ts",
			text: "edit src/main.py",
		},
		{
			name: "web_search query",
			code: 'web_search("python asyncio best practices")',
			kind: "ts",
			text: "web_search python asyncio best practices",
		},
		{
			name: "f-string command resolves",
			code: 'target = "src"\nbash(f"ls -la {target}")',
			kind: "shell",
			text: "ls -la src",
		},
		{
			name: "variable path resolves",
			code: 'path = "README.md"\ncontent = read(path)\ncontent[:100]',
			kind: "ts",
			text: "read README.md",
		},
		{
			name: "open(..., 'w') previews as write",
			code: 'with open("/tmp/out.txt", "w") as f: f.write("x")',
			kind: "ts",
			text: "write /tmp/out.txt",
		},
		{
			name: "generic call beats import and blank",
			code: "import os\n\nprint(analyze(data))",
			kind: "ts",
			text: "analyze(data)",
		},
		{
			name: "empty code is empty",
			code: "",
			kind: "ts",
			text: "",
		},
